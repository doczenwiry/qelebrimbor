...
if __name__ == "__main__":
    pyzx_graph = zx.Graph()
    with open("../assets/zx/steane-code-qubits7-volume14.json", 'r') as file:
    ...
    cycle2 = cycles[2]
    n2 = len(cycle2)
    nodes2 = list(map(lambda nd : azx.get_node_type(nd), cycle2))
    console.info(f"Nodes 2 : {" ".join(map(lambda nd: str(nd) + ':' + str(azx.get_node_type(nd)), cycle2))}")
    ...
    # Breakdown cycle1
    start = min(azx.get_realising_cubes(1))
    final = min(azx.get_realising_cubes(6))
    ...
    final_cube_idx = 3
    start = min(azx.get_realising_cubes(0))
    final = list(azx.get_realising_cubes(1))[final_cube_idx]
    start_cube = (azx.get_cube_kind(start), azx.get_cube_position(start))
    final_cube = (azx.get_cube_kind(final), azx.get_cube_position(final))
    console.info(f"Searching completion from 0 #{start} [{start_cube}] to 1 #{final} [{final_cube}]")
    minimal_overhead, completions2 = PathFinderDFS.find_minimal_paths(
    ...
    BlockGraphConstructor.realise_edges(azx, {
        (0, 1): PathSpecification(
            ...
            target_cube = list(azx.get_realising_cubes(1))[final_cube_idx],
            extras = completion2.cubes[1:c2 - 1],
            pipes = [EdgeType.IDENTITY for _ in range(n2+2)]
        )
    })

    console.info(f"TOTAL VOLUME : {azx.number_of_cubes()}")

    hexagon = prepare_layout()
    viewer = AugmentedZxGraphViewer(azx, label = "steane-code", layout = hexagon)
    viewer.display()