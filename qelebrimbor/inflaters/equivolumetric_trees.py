...
class ZxGraphInflaterEquivolumetricTrees:
    ...
    def __obtain_available_port(self, graph: VolumetricZxGraph, current: ZxNode) -> tuple[BgCube, Port] | None:
        ...
        console.debug(f">> Equivalents : {graph.get_equivalent_bg_cubes(cube)}")
        ...
        for tree in trees:
            ...
            for current in tree.level(level):
                ...
                if not current.is_realised():
                    ...
                    edge = graph.get_zx_edge(preceding_node.id, current.id)
                    ...
                    if available_port is not None:
                        ...
                        cube_id = graph.realise_zx_node(current, cube=cube)
                ...
                required = self.__count_required_ports(graph, current)
                available = self.__count_available_ports(graph, current)
                console.debug(f">> Checking node {current} : {required} / {available} ")
                if required > available:
                    available_port = self.__obtain_available_port(graph, current)
                    console.debug(f">>> Node {current} has degree {graph.get_zx_degree(current.id)}")
                    if available_port is not None:
                        current_cube, port = available_port
                        cube = BgCube(kind=current_cube.kind, position=port)
                        cube_id = graph.extend_zx_node(current, cube=current_cube, extension=cube)
                    else:
