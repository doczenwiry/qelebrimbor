...
if __name__ == "__main__":
    ...
    BlockGraphConstructor.realise_edges(
        graph=vzx,
        specifications={
            (1, 5): Path(vzx.get_zx_node(1).realising_cube)
            .extend(
                cube=BgCube(kind=CubeKind.XZZ, position=Coordinates(0, 0, 1)),
                pipe_type=vzx.get_zx_edge(1, 5).type,
            )
            .extend(cube=vzx.get_zx_node(5).realising_cube, pipe_type=EdgeType.IDENTITY),
            (2, 7): Path(vzx.get_zx_node(2).realising_cube)
            .extend(
                cube=BgCube(kind=CubeKind.ZXX, position=Coordinates(-1, 1, 2)),
                pipe_type=vzx.get_zx_edge(2, 7).type,
            )
            .extend(cube=vzx.get_zx_node(7).realising_cube, pipe_type=EdgeType.IDENTITY),
        },
    )
    ...
    BlockGraphConstructor.realise_edges(
        graph=vzx,
        specifications={
            (3, 6): Path(vzx.get_zx_node(3).realising_cube)
            .extend(
            ...
            .extend(cube=vzx.get_zx_node(6).realising_cube, pipe_type=EdgeType.IDENTITY),
        },
    )
    ...
    # PYZX.into_file(
    #     vzx,
    #     filepath="../assets/pyzx/steane/steane-code-qubits7-spiders8-alt-blockgraph.json",
    # )
